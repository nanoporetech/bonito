...
Bonito tuning.
  ...
  $ bonito tune /tmp/bonito-tune config/quartznet5x5.toml
...
def main(args):
    ...
    def objective(trial):
        ...
        # B1 - B5
        for i in range(1, 6):
            config['block'][i]['repeat'] = trial.suggest_int('b%s_repeat' % i, 1, 6)
            config['block'][i]['filters'] = trial.suggest_int('b%s_filters' % i, 1, 512)
            config['block'][i]['kernel'] = [int(trial.suggest_discrete_uniform('b%s_kernel' %i, 1, 99, 2))]

        # C2
        config['block'][-2]['kernel'] = [int(trial.suggest_discrete_uniform('c2_kernel', 1, 99, 2))]
...
def argparser():
    ...
    parser.add_argument("--max-params", default=20000000, type=int)
